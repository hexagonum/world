...
const buildFilterOptions = (
  ...
  const filteredCountries: string[] = countries
    .filter(
      ({ region, subregion }) =>
        (filterOption.region !== '' ? filterOption.region === region : true) &&
        (filterOption.subregion !== ''
          ? filterOption.subregion === subregion
          : true)
    )
...
const GoogleTrends: React.FC<GoogleTrendsProps> = ({
  filterOption,
  initialCountries = [],
}) => {
  const { loading, error, data } = useQuery<{ countries: Country[] }>(
    COUNTRIES_GOOGLE_TRENDS_QUERY
  );
      ...
      .filter(({ region, subregion, commonName }) => {
        ...
        const countryFlag: boolean =
          filterOption.country !== ''
            ? filterOption.country === commonName
            : true;
...
const GoogleTrendsPage: NextPage<GoogleTrendsPageProps> = ({
  countries = [],
}) => {
  // Filter
  const router = useRouter();
...
export const getStaticProps = async (): Promise<{
  ...
  try {
    ...
    const countries: Country[] = [...data.data.countries].filter(
