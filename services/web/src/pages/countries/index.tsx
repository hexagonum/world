...
export const CountriesPage: NextPage<CountriesPageProps> = ({
  ...
  const countriesByFilter = countries.filter(
    ({
      ...
      const fifaFlag: boolean =
        query !== '' ? fifa.toLowerCase().includes(query.toLowerCase()) : true;
      ...
      const subregionFlag: boolean =
        query !== ''
          ? subregion.toLowerCase().includes(query.toLowerCase())
          : true;
      return (
        cca2Flag ||
        cca3Flag ||
  ...
  const regions: string[] = [
    ...new Set(countriesByFilter.map(({ region = '' }) => region)),
  ...
  const countriesByRegions: {
    ...
    const subregions: string[] = unique(
      countriesByRegions.map(({ subregion = '' }) => subregion)
    ).sort((a: string, b: string) => (a > b ? 1 : -1));
    const countriesBySubregions: { subregion: string; countries: Country[] }[] =
      subregions.map((subregion: string) => {
        const countries: Country[] = countriesByRegions
          .filter(
            ({ subregion: countrySubregion }: Country) =>
    ...
    return {
      region,
      total: countriesByRegions.length,
      subregions: countriesBySubregions,
    };
  });
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Link href={`/countries/${cca3}`}>
                                        <pre className="inline">{cca2}</pre> -{' '}
                                        <pre className="inline">{cca3}</pre> -{' '}
