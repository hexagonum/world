...
export const OrganizationsPage: NextPage<OrganizationsPageProps> = ({
  countries = [],
}) => {
  ...
  const countriesByFilter = countries.filter(
    ({ commonName = '', organizations = [] }) => {
      const organizationCodes: string[] = organizations.map(
        ({ code = '' }: { code: string }) => code
      );
      const organizationNames: string[] = organizations.map(
        ({ name = '' }: { name: string }) => name
      );
      const codeFlag: boolean =
        query !== ''
          ? organizationCodes.some((code: string) =>
              code.toLowerCase().includes(query.toLowerCase())
            )
          : true;
      const nameFlag: boolean =
        query !== ''
          ? organizationNames.some((name: string) =>
            ...
            <div className="shadow">
              <TableContainer className="border rounded">
                <Table>
                  ...
                  <TableCaption>
                    <p className="pb-4">
                      Organizations ({countriesByFilter.length})
                    </p>
                  </TableCaption>
                </Table>
              </TableContainer>
