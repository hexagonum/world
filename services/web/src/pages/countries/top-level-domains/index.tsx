...
export const TopLevelDomainsPage: NextPage<TopLevelDomainsPageProps> = ({
  countries = [],
}) => {
  const [query, setQuery] = useState<string>('');

  const countriesByFilter = countries.filter(
    ({ commonName = '', topLevelDomains = [] }) => {
      const topLevlDomainsFlag: boolean =
        query !== ''
          ? topLevelDomains.some((code: string) =>
            ...
            <div className="shadow">
              <TableContainer className="border rounded">
                <Table>
                  ...
                  <TableCaption>
                    <p className="pb-4">
                      Top-level Domains ({countriesByFilter.length})
                    </p>
                  </TableCaption>
                </Table>
              </TableContainer>
...
export const getStaticProps: GetStaticProps = async (): Promise<{
  ...
  try {
    ...
    countries.sort(
      (a, b) => b.topLevelDomains.length - a.topLevelDomains.length
    );
